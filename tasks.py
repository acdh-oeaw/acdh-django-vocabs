...
def docs(c):
    ...
    """
    c.run("rm -f docs/acdh-django-vocabs.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ vocabs")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=acdh_django_vocabs.__version__))
    c.run("git push --tags")
